...
static const Vector *SelectLearnedCampSpot(CBaseEntity *me, CollectHidingSpotsFunctor &collector, const Vector *pos, bool useNearest)
{
	if (collector.m_count <= 0)
		return nullptr;

	const Vector *bestBadSpot = nullptr;
	float bestBadWeight = 999999.0f;
	const Vector *usable[CollectHidingSpotsFunctor::MAX_SPOTS];
	int usableCount = 0;

	for (int i = 0; i < collector.m_count; ++i)
	{
		const Vector *spot = collector.m_hidingSpot[i];
		const float badWeight = BotCampMemory::GetBadCampWeight(me, *spot);
		if (badWeight <= 0.0f || !BotCampMemory::IsBadCampSpot(me, *spot))
		{
			usable[usableCount++] = spot;
			continue;
		}

		if (badWeight < bestBadWeight)
		{
			bestBadWeight = badWeight;
			bestBadSpot = spot;
		}
	}

	if (usableCount <= 0)
		return bestBadSpot ? bestBadSpot : collector.m_hidingSpot[RANDOM_LONG(0, collector.m_count - 1)];
	...
	if (useNearest)
	{
		...
		{
			float rangeSq = (*usable[i] - *pos).LengthSquared();
	...
	return usable[RANDOM_LONG(0, usableCount - 1)];
}
...
const Vector *FindNearbyHidingSpot(CBaseEntity *me, const Vector *pos, CNavArea *startArea, float maxRange, bool isSniper, bool useNearest)
{
	...
	if (isSniper)
	{
		...
		{
			return SelectLearnedCampSpot(me, collector, pos, false);
		}
		else
		{
	...
	if (useNearest)
	{
		return SelectLearnedCampSpot(me, collector, pos, true);
	}
