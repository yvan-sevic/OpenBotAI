...
void BotProfileManager::Init(const char *filename, unsigned int *checksum)
{
	...
	while (true)
	{
		...
		while (true)
		{
			...
			if (!dataFile)
			{
				CONSOLE_ECHO("Error parsing %s - expected '=' after '%s'\n", filename, attributeName);
				FREE_FILE(dataPointer);
				return;
			}
			...
			if (Q_strcmp(token, "=") != 0)
			{
				CONSOLE_ECHO("Error parsing %s - expected '=' after '%s' (bot name line: use \"quotes\" if the name has multiple words)\n", filename, attributeName);
				FREE_FILE(dataPointer);
				return;
			}
			...
			else if (!Q_stricmp("ChatPersona", attributeName))
			{
				// Optional mod field: short character/voice hint (single token; use underscores for spaces).
				if (profile->m_chatPersona)
				{
					delete[] profile->m_chatPersona;
					profile->m_chatPersona = nullptr;
				}

				if (token[0] && Q_stricmp(token, "none"))
					profile->m_chatPersona = CloneString(token);
			}
			else
			{
