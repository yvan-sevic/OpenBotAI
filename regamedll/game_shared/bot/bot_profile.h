...
public:
	...
	~BotProfile()
	{
		delete[] m_name;
		m_name = nullptr;
		delete[] m_chatPersona;
		m_chatPersona = nullptr;
	}

	const char *GetName() const { return m_name; }
	// Optional short bio / voice character line for mods (e.g. AI chat). Parsed from BotProfile.db as ChatPersona = ...
	const char *GetChatPersona() const { return m_chatPersona; }
...
inline void BotProfile::Inherit(const BotProfile *parent, const BotProfile *baseline)
{
	...
	// Optional string: only override when the parent template defines one (nullptr = leave previous inheritance).
	if (parent->m_chatPersona && parent->m_chatPersona[0])
	{
		if (m_chatPersona)
			delete[] m_chatPersona;
		m_chatPersona = CloneString(parent->m_chatPersona);
	}
}
