...
// AI bot chat via Ollama (async HTTP). Set to 0 to disable.
cvar_t sv_ai_chat_enable     = { "sv_ai_chat_enable", "1", FCVAR_SERVER, 0.0f, nullptr };
// Ollama settings (when sv_ai_chat_enable is non-zero)
cvar_t sv_ai_ollama_url      = { "sv_ai_ollama_url", "http://127.0.0.1:11434", FCVAR_SERVER, 0.0f, nullptr };
cvar_t sv_ai_ollama_model    = { "sv_ai_ollama_model", "llama3", FCVAR_SERVER, 0.0f, nullptr };
cvar_t sv_ai_timeout_ms      = { "sv_ai_timeout_ms", "1500", FCVAR_SERVER, 0.0f, nullptr };
cvar_t sv_ai_max_chars       = { "sv_ai_max_chars", "120", FCVAR_SERVER, 0.0f, nullptr };
cvar_t sv_ai_debug           = { "sv_ai_debug", "0", FCVAR_SERVER, 0.0f, nullptr };
// "auto" (default): bombs to team, round start + kills to all. "all": always all chat. "team": always team chat.
cvar_t sv_ai_chat_scope      = { "sv_ai_chat_scope", "auto", FCVAR_SERVER, 0.0f, nullptr };
// Optional prefix to visually mark AI team-only messages while still using "#Cstrike_Chat_All" token.
cvar_t sv_ai_chat_team_tag   = { "sv_ai_chat_team_tag", "0", FCVAR_SERVER, 0.0f, nullptr };
// Aim comparison logs for tuning bot humanization. Writes aim_bot_events.csv and aim_human_events.csv in the game dir (GET_GAME_DIR, e.g. cstrike/) when enabled.
cvar_t sv_aim_log_enable     = { "sv_aim_log_enable", "0", FCVAR_SERVER, 0.0f, nullptr };
cvar_t sv_aim_log_interval   = { "sv_aim_log_interval", "0.10", FCVAR_SERVER, 0.0f, nullptr };
cvar_t sv_roam_aim_log_enable = { "sv_roam_aim_log_enable", "0", FCVAR_SERVER, 0.0f, nullptr };
// Human grenade throws -> grenade_human_events.csv in game dir (GET_GAME_DIR); use scripts/grenade_human_log_to_json.py to build maps/<map>.grenade.json
cvar_t sv_grenade_throw_log_enable = { "sv_grenade_throw_log_enable", "0", FCVAR_SERVER, 0.0f, nullptr };
// Runtime-learned bot danger memory. Writes maps/<map>.danger.json when learning is enabled.
cvar_t bot_danger_memory_enable = { "bot_danger_memory_enable", "1", FCVAR_SERVER, 0.0f, nullptr };
cvar_t bot_danger_memory_learn  = { "bot_danger_memory_learn", "1", FCVAR_SERVER, 0.0f, nullptr };
cvar_t bot_danger_memory_debug  = { "bot_danger_memory_debug", "0", FCVAR_SERVER, 0.0f, nullptr };
// Runtime-learned bad camp memory. Writes maps/<map>.camp.json when learning is enabled.
cvar_t bot_camp_memory_enable   = { "bot_camp_memory_enable", "1", FCVAR_SERVER, 0.0f, nullptr };
cvar_t bot_camp_memory_learn    = { "bot_camp_memory_learn", "1", FCVAR_SERVER, 0.0f, nullptr };
cvar_t bot_camp_memory_debug    = { "bot_camp_memory_debug", "0", FCVAR_SERVER, 0.0f, nullptr };
cvar_t bot_grenade_spots_debug  = { "bot_grenade_spots_debug", "0", FCVAR_SERVER, 0.0f, nullptr };
// Debug mode for HE-only tuning: bots always buy HE grenades and never buy flash/smoke.
cvar_t bot_grenade_debug_he_only = { "bot_grenade_debug_he_only", "0", FCVAR_SERVER, 0.0f, nullptr };
// When maps/<map>.grenade.json has HE lineups for the bot's team, try danger.json HE only with this probability after manual miss (0=never, 1=always). Only if bot_grenade_he_use_danger_memory is 1.
cvar_t bot_grenade_he_danger_fallback = { "bot_grenade_he_danger_fallback", "0.32", FCVAR_SERVER, 0.0f, nullptr };
// If 0 (default), HE targets use only grenade.json / path toss / throw-at-threat — not maps/<map>.danger.json.
cvar_t bot_grenade_he_use_danger_memory = { "bot_grenade_he_use_danger_memory", "0", FCVAR_SERVER, 0.0f, nullptr };
...
void EXT_FUNC GameDLLInit()
{
	...
	CVAR_REGISTER(&sv_ai_chat_enable);
	CVAR_REGISTER(&sv_ai_ollama_url);
	CVAR_REGISTER(&sv_ai_ollama_model);
	CVAR_REGISTER(&sv_ai_timeout_ms);
	CVAR_REGISTER(&sv_ai_max_chars);
	CVAR_REGISTER(&sv_ai_debug);
	CVAR_REGISTER(&sv_ai_chat_scope);
	CVAR_REGISTER(&sv_ai_chat_team_tag);
	CVAR_REGISTER(&sv_aim_log_enable);
	CVAR_REGISTER(&sv_aim_log_interval);
	CVAR_REGISTER(&sv_roam_aim_log_enable);
	CVAR_REGISTER(&sv_grenade_throw_log_enable);
	CVAR_REGISTER(&bot_danger_memory_enable);
	CVAR_REGISTER(&bot_danger_memory_learn);
	CVAR_REGISTER(&bot_danger_memory_debug);
	CVAR_REGISTER(&bot_camp_memory_enable);
	CVAR_REGISTER(&bot_camp_memory_learn);
	CVAR_REGISTER(&bot_camp_memory_debug);
	CVAR_REGISTER(&bot_grenade_spots_debug);
	CVAR_REGISTER(&bot_grenade_debug_he_only);
	CVAR_REGISTER(&bot_grenade_he_danger_fallback);
	CVAR_REGISTER(&bot_grenade_he_use_danger_memory);
