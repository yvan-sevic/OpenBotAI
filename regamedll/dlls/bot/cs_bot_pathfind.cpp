...
void CCSBot::SetupLadderMovement()
{
	...
	if (to->ladder)
	{
		...
		if (to->how == GO_LADDER_UP)
		{
			...
			Vector g = m_pathLadder->m_bottom;
			AddDirectionVector(&g, m_pathLadder->m_dir, HalfHumanWidth * 2.0f);
			AssignMovementGoalPosition(g, MoveDbgGoalSource::LadderApproach, __FUNCTION__);
			m_lookAheadAngle = DirectionToAngle(OppositeDirection(m_pathLadder->m_dir));
		}
		else
		{
			// try to mount ladder "face out" first
			Vector g = m_pathLadder->m_top;
			AddDirectionVector(&g, OppositeDirection(m_pathLadder->m_dir), HalfHumanWidth * 2.0f);

			TraceResult result;
			Vector from = m_pathLadder->m_top;
			Vector traceEnd = g;

			UTIL_TraceLine(from, traceEnd, ignore_monsters, ENT(m_pathLadder->m_entity->pev), &result);
			...
			else
			{
				...
				m_pathLadderFaceIn = true;
				m_lookAheadAngle = DirectionToAngle(OppositeDirection(m_pathLadder->m_dir));
				Vector g2 = m_pathLadder->m_top;
				AddDirectionVector(&g2, m_pathLadder->m_dir, HalfHumanWidth);
				AssignMovementGoalPosition(g2, MoveDbgGoalSource::LadderApproach, __FUNCTION__);
			}
		}
	}
}
...
bool CCSBot::UpdateLadderMovement()
{
	...
	if (giveUp)
	{
		...
		Wiggle();
		ResetStuckMonitor();
		DestroyPath(MoveDbgPathClearReason::LadderFail, __FUNCTION__);
		Run();
		return false;
	}
	...
	case APPROACH_DESCENDING_LADDER:
	{
		...
		if (GetFeetZ() <= m_pathLadder->m_bottom.z + HalfHumanHeight)
		{
			...
			m_pathLadderState = MOVE_TO_DESTINATION;
			Vector g;
			m_path[m_pathIndex].area->GetClosestPointOnArea(&m_pathLadder->m_bottom, &g);
			AddDirectionVector(&g, m_pathLadder->m_dir, HalfHumanWidth);
			AssignMovementGoalPosition(g, MoveDbgGoalSource::LadderMoveToDestination, __FUNCTION__);
	...
	case ASCEND_LADDER:
	{
		...
		else if (!IsOnLadder())
		{
			...
			DestroyPath(MoveDbgPathClearReason::LadderFallOff, __FUNCTION__);
			return false;
		}
	...
	case DISMOUNT_ASCENDING_LADDER:
	{
		if (gpGlobals->time - m_pathLadderDismountTimestamp >= 0.4f)
		{
			m_pathLadderState = MOVE_TO_DESTINATION;
			Vector g;
			m_path[m_pathIndex].area->GetClosestPointOnArea(&pev->origin, &g);
			AssignMovementGoalPosition(g, MoveDbgGoalSource::LadderDismount, __FUNCTION__);
			PrintIfWatched("MOVE_TO_DESTINATION\n");
		}
...
CCSBot::PathResult CCSBot::UpdatePathMovement(bool allowSpeedChange)
{
	...
	if (m_pathIndex >= m_pathLength - 1)
	{
		...
		if (d.IsLengthLessThan(walkRange))
		{
			...
			if (allowSpeedChange && !IsCrouching() && !IsNearJump() && d.z < StepHeight)
				Walk();
			...
			if (d.IsLengthLessThan(closeEpsilon))
			{
				...
				DestroyPath(MoveDbgPathClearReason::ReachedGoal, __FUNCTION__);
	...
	// Movement vs look: m_goalPosition / path drive locomotion; the block below sets default
	// path-following look intent (m_lookAheadAngle, m_lookAheadPitch). Actual view targets also come
	// from SetLookAt / UpdateLookAround (cs_bot_vision.cpp) and combat aim (cs_bot_aim_controller.cpp).
	...
	else if (m_pathIndex < m_pathLength)
	{
		...
		if (gpGlobals->time >= m_nextPathLookAheadRangeUpdate)
		{
			m_pathLookAheadRange = RANDOM_FLOAT(360.0f, 640.0f);

			// Sometimes people glance closer around corners, sometimes farther down a lane.
			if (RANDOM_FLOAT(0.0f, 100.0f) < 14.0f)
				m_pathLookAheadRange = RANDOM_FLOAT(260.0f, 380.0f);

			m_nextPathLookAheadRangeUpdate = gpGlobals->time + RANDOM_FLOAT(0.75f, 1.75f);
		}

		const float lookAheadRange = m_pathLookAheadRange;
	...
	if (toGoal.Length2D() > 32.0f)
	{
		Vector lookAhead = toGoal;
		lookAhead.z += HalfHumanHeight;
		Vector idealAngle = UTIL_VecToAngles(lookAhead);
		float terrainPitch = NormalizeAngle(360.0f - idealAngle.x);

		// Aim into the lane above the slope, not down at the nav floor point itself.
		const float lookAheadRange2D = toGoal.Length2D();
		if (terrainPitch > 0.0f && lookAheadRange2D < 180.0f)
			terrainPitch *= lookAheadRange2D / 180.0f;

		if (terrainPitch < -8.0f)
			terrainPitch = -8.0f;
		else if (terrainPitch > 3.5f)
			terrainPitch = 3.5f;

		m_lookAheadPitch = 0.74f * m_lookAheadPitch + 0.26f * terrainPitch;
	}
	else
	{
		m_lookAheadPitch *= 0.78f;
	}
	...
	if (!IsAttacking() && IsFriendInTheWay(&m_goalPosition))
	{
		...
		else if (m_politeTimer.IsElapsed())
		{
			...
			DestroyPath(MoveDbgPathClearReason::FriendAvoidRepath, __FUNCTION__);
		}
	}
	...
	if (didFall || gpGlobals->time - m_areaEnteredTimestamp > giveUpDuration)
	{
		...
		Run();
		StandUp();
		DestroyPath(MoveDbgPathClearReason::StuckTimeout, __FUNCTION__);
...
void CCSBot::BuildTrivialPath(const Vector *goal)
{
	...
	AssignMovementGoalPosition(*goal, MoveDbgGoalSource::BuildTrivialPath, __FUNCTION__);
}
...
bool CCSBot::ComputePath(CNavArea *goalArea, const Vector *goal, RouteType route)
{
	...
	DestroyPath(MoveDbgPathClearReason::RepathCompute, __FUNCTION__);
	...
	if (ComputePathPositions() == false)
	{
		...
		DestroyPath(MoveDbgPathClearReason::PathFailure, __FUNCTION__);
		return false;
	}
	...
	m_areaEnteredTimestamp = gpGlobals->time;
	m_spotEncounter = nullptr;
	AssignMovementGoalPosition(m_path[1].pos, MoveDbgGoalSource::ComputePathWaypoint, __FUNCTION__);
