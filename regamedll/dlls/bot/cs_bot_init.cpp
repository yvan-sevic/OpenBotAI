...
// 1..15 base skill (effective = base + Director offset, clamped 0..15). Integer 0..3 still accepted as legacy easy→expert spread.
cvar_t cv_bot_difficulty             = { "bot_difficulty", "5", FCVAR_SERVER, 0.0f, nullptr };
// bot_debug: CBot::PrintIfWatched console text. 1 or 3 = when spectating this bot; 2 or 4 = all bots.
// Reserved: 6 = future opt-in aim-pipeline trace (not wired yet; avoid noisy per-tick logging).
cvar_t cv_bot_debug                  = { "bot_debug", "0", FCVAR_SERVER, 0.0f, nullptr };
// When non-zero, [movedbg] / [OSCILLATION SUSPECT] lines are also appended to bot_move_debug_logfile under the game dir.
cvar_t cv_bot_move_debug_log         = { "bot_move_debug_log", "0", FCVAR_SERVER, 0.0f, nullptr };
cvar_t cv_bot_move_debug_logfile     = { "bot_move_debug_logfile", "bot_move_debug.log", FCVAR_SERVER, 0.0f, nullptr };
cvar_t cv_bot_director_debug         = { "bot_director_debug", "1", FCVAR_SERVER, 0.0f, nullptr };
...
void Bot_RegisterCVars()
{
	...
	CVAR_REGISTER(&cv_bot_move_debug_log);
	CVAR_REGISTER(&cv_bot_move_debug_logfile);
	CVAR_REGISTER(&cv_bot_director_debug);
...
void CCSBot::ResetValues()
{
	...
	m_nextTacticalGrenadeTimestamp = 0.0f;
	m_grenadeHeldNoThrowSince = 0.0f;
	m_openingGrenadeChecked = false;
	m_openingGrenadeWantsThrow = false;
	m_openingGrenadeTimestamp = 0.0f;
	m_grenadeThrowTarget = Vector(0, 0, 0);
	...
	m_boltSniperQuickSwitchAwayTimestamp = 0.0f;
	m_boltSniperQuickSwitchReturnTimestamp = 0.0f;
	...
	m_lastIdleLookYaw = 0.0f;
	m_prevIdleLookYaw = 0.0f;
	m_suppressAvoidWallStareUntil = 0.0f;
	m_nextAvoidWallStareSkipLogTime = 0.0f;
	m_nextWallLookClassifyLogTime = 0.0f;
	m_lookLosRejectLogNextTime = 0.0f;
	m_campLookRotateCursor = 0;
	m_roamLookPhase = ROAM_LOOK_FORWARD;
	m_roamLookPhaseTimestamp = 0.0f;
	m_roamPoiHoldActive = false;
	m_roamPoiHoldEndTime = 0.0f;
	m_roamPoiHoldAimPos = Vector(0, 0, 0);
	m_roamPoiHoldSource = 0;
	m_roamPoiHoldScore = 0.0f;
	m_roamPoiHoldCommitOrigin = Vector(0, 0, 0);
	m_roamPoiHoldDebugRemain = -1.0f;
	m_roamPoiHoldDebugYieldTag = nullptr;
	m_lookAheadPitch = 0.0f;
	m_pathLookAheadRange = 500.0f;
	m_nextPathLookAheadRangeUpdate = 0.0f;

	m_lookPitch = 0.0f;
	m_lookPitchVel = 0.0f;
	m_lookYaw = 0.0f;
	m_lookYawVel = 0.0f;
	m_nonCombatLookYawOffset = 0.0f;
	m_nonCombatLookPitchOffset = 0.0f;
	m_nextNonCombatLookOffsetUpdate = 0.0f;
	m_nonCombatLookMotorScale = 1.0f;
	m_nextNonCombatLookMotorUpdate = 0.0f;
	m_nonCombatLookPitchBias = 0.0f;
	m_nextNonCombatLookPitchBiasUpdate = 0.0f;

	m_aimOffsetTimestamp = 0.0f;
	m_aimSpreadTimestamp = 0.0f;
	m_perceivedEnemyPosition = Vector(0, 0, 0);
	m_perceivedEnemyVelocity = Vector(0, 0, 0);
	m_aimInertiaSpot = Vector(0, 0, 0);
	...
	m_combatAimPhaseTimestamp = 0.0f;
	m_combatAimPhaseEndTimestamp = 0.0f;
	m_combatAimOvershoot = Vector(0, 0, 0);
	m_combatAimConfidence = 0.0f;
	m_perceivedEnemyIndex = 0;
	m_aimDiagMotorStep.valid = false;
	m_aimDiagJitterMag = 0.0f;
	m_aimDiagAimOffsetMag = 0.0f;
	m_aimDiagRecoilMag = 0.0f;
	m_aimDiagDistEnemy = 0.0f;
	m_aimDiagRefusedAimCone = false;
	m_aimDiagAcquisitionBurst = false;
	m_aimDiagPrintNextTime = 0.0f;
