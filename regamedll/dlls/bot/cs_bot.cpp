...
static const float kCombatTargetCommitDurationSecs = 0.25f;
...
static bool CanUseQuietWalk(CCSBot *bot)
{
	if (!bot)
		return false;
	...
	if (bot->IsNoiseHeard() && noise && (bot->pev->origin - *noise).IsLengthLessThan(1200.0f))
		return true;
...
void CCSBot::Walk()
{
	if (!m_mustRunTimer.IsElapsed() || !CanUseQuietWalk(this))
	{
		Run();
		return;
	}
		...
		if (IsEnemy(pPlayer))
		{
			...
			if (!IsSurprised() && (m_attacker != lastAttacker || m_attackedTimestamp != lastAttackedTimestamp))
			{
				...
				if (!IsVisible(pPlayer, CHECK_FOV))
				{
					...
					if (!bPanic)
					{
						float invSkill = 1.0f - TheCSBots()->GetEffectiveSkill(this);
...
static bool NoiseHumanReactionSkipForObjectives(const CCSBot *me)
{
	if (!me)
		return true;
...
void CCSBot::AssignNoiseHumanReactionDelayOnStimulus(PriorityType priority)
{
	...
	{
		Q_strncpy(m_noiseHumanDelayDebugWhy, "skip_urgent_pri", sizeof m_noiseHumanDelayDebugWhy);
		m_noiseHumanDelayDebugWhy[sizeof(m_noiseHumanDelayDebugWhy) - 1] = '\0';
		return;
	}

	if (IsEnemyVisible())
	{
		Q_strncpy(m_noiseHumanDelayDebugWhy, "skip_visible_enemy", sizeof m_noiseHumanDelayDebugWhy);
		m_noiseHumanDelayDebugWhy[sizeof(m_noiseHumanDelayDebugWhy) - 1] = '\0';
		return;
	}
	...
	if (IsSurprised())
	{
		Q_strncpy(m_noiseHumanDelayDebugWhy, "skip_surprised", sizeof m_noiseHumanDelayDebugWhy);
		m_noiseHumanDelayDebugWhy[sizeof(m_noiseHumanDelayDebugWhy) - 1] = '\0';
		return;
	}

	if (m_attackedTimestamp > 0.0f && GetTimeSinceAttacked() < 1.0f)
	{
		Q_strncpy(m_noiseHumanDelayDebugWhy, "skip_recent_damage", sizeof m_noiseHumanDelayDebugWhy);
		m_noiseHumanDelayDebugWhy[sizeof(m_noiseHumanDelayDebugWhy) - 1] = '\0';
		return;
	}

	if (NoiseHumanReactionSkipForObjectives(this))
	{
		Q_strncpy(m_noiseHumanDelayDebugWhy, "skip_objective", sizeof m_noiseHumanDelayDebugWhy);
		m_noiseHumanDelayDebugWhy[sizeof(m_noiseHumanDelayDebugWhy) - 1] = '\0';
		return;
	}
	...
	{
		Q_strncpy(m_noiseHumanDelayDebugWhy, "skip_pri_unknown", sizeof m_noiseHumanDelayDebugWhy);
		m_noiseHumanDelayDebugWhy[sizeof(m_noiseHumanDelayDebugWhy) - 1] = '\0';
		return;
	}
	...
	float r = RANDOM_FLOAT(minD, maxD);
	const float adjust = (1.0f - skill) * 0.04f + (1.0f - aggression) * 0.03f - aggression * 0.04f;
	r += adjust;
	r = Q_max(minD, Q_min(maxD, r));

	m_noiseHumanDelay = r;
	m_noiseHumanDelayApplied = true;

	if (priority == PRIORITY_LOW)
		Q_strncpy(m_noiseHumanDelayDebugWhy, "applied_pri_low", sizeof m_noiseHumanDelayDebugWhy);
	else
		Q_strncpy(m_noiseHumanDelayDebugWhy, "applied_pri_medium", sizeof m_noiseHumanDelayDebugWhy);
...
void CCSBot::SetEnemy(CBasePlayer *pEnemy)
{
	if (m_enemy != pEnemy)
	{
		...
		m_perceivedEnemyTimestamp = 0.0f;
		m_nextPerceivedEnemyUpdate = 0.0f;
		m_aimInertiaTimestamp = 0.0f;
		m_aimHandJitter = Vector(0, 0, 0);
		m_nextAimJitterUpdate = 0.0f;
		m_aimCurveOffset = Vector(0, 0, 0);
		m_nextAimCurveUpdate = 0.0f;
		m_aimHumanMotorScale = 1.0f;
		m_nextAimMotorUpdate = 0.0f;
		m_recoilControlOffset = Vector(0, 0, 0);
		m_recoilControlStartTimestamp = 0.0f;
		m_recoilControlLastFireTimestamp = 0.0f;
		m_nextRecoilControlUpdate = 0.0f;
		m_recoilControlSide = 1.0f;
		m_recoilControlWeapon = WEAPON_NONE;
		m_combatAimPhase = pEnemy ? COMBAT_AIM_REACTING : COMBAT_AIM_TRACKING;
		...
		m_perceivedEnemyIndex = pEnemy ? pEnemy->entindex() : 0;
...
#endif
		if (pEnemy && gpGlobals)
			m_combatTargetCommitUntil = gpGlobals->time + kCombatTargetCommitDurationSecs;
		else
		{
			m_combatTargetCommitUntil = 0.0f;
			m_combatTargetCommitDbgNextLog = 0.0f;
...
void CCSBot::Panic(CBasePlayer *pEnemy)
{
	...
	if (TheCSBots()->GetEffectiveSkill(this) >= 0.5f)
	{
...
void CCSBot::GetOffLadder()
{
	...
	{
		Jump(MUST_JUMP);
		DestroyPath(MoveDbgPathClearReason::GetOffLadder, __FUNCTION__);
	}
}
...
// Return true if this bot personally wants to switch to knife during safe time
bool CCSBot::ShouldEquipKnifeForSafeRun()
{
	if (!IsSafe())
	{
		m_safeTimeKnifeSwitchTimestamp = 0.0f;
		m_safeTimeWantsKnife = false;
		return false;
	}

	if (m_safeTimeKnifeSwitchTimestamp == 0.0f)
	{
		float knifeChance = 30.0f + 25.0f * TheCSBots()->GetEffectiveSkill(this) + 15.0f * GetProfile()->GetAggression();
		...
		knifeChance = Q_max(18.0f, Q_min(knifeChance, 78.0f));
		m_safeTimeWantsKnife = (RANDOM_FLOAT(0.0f, 100.0f) < knifeChance);

		const float remainingSafeTime = Q_max(0.0f, GetSafeTimeRemaining());
		const float latestSwitchDelay = Q_min(3.2f, Q_max(0.35f, remainingSafeTime - 1.0f));
		m_safeTimeKnifeSwitchTimestamp = gpGlobals->time + RANDOM_FLOAT(0.15f, latestSwitchDelay);
	}

	return m_safeTimeWantsKnife && gpGlobals->time >= m_safeTimeKnifeSwitchTimestamp;
}
