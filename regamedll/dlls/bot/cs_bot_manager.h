...
public:
	...
	// difficulty levels
	int GetBaseDifficulty15() const;
	int GetEffectiveDifficulty15() const;
	float GetEffectiveDifficultyFraction() const;
	float GetEffectiveSkill(CCSBot *bot) const;
	BotDifficultyType GetDifficultyLevel() const;
	// BotProfile.db selection uses bot_difficulty (base), not Director adaptive skill.
	BotDifficultyType GetProfileDifficultyLevel() const;
	...
	bool ShouldDefenderRush(CCSBot *bot) const;							// return true if this defender joins the local rush group
	bool ShouldJoinTeamRush(CCSBot *bot, CBasePlayer *caller) const;		// return true if this bot personally follows a rush command
	...
	enum DirectorRoleType
	{
		DIRECTOR_ROLE_NONE = 0,
		DIRECTOR_ROLE_ENTRY,
		DIRECTOR_ROLE_SUPPORT,
		DIRECTOR_ROLE_LURK,
		DIRECTOR_ROLE_ANCHOR,
		DIRECTOR_ROLE_ROTATOR,
		DIRECTOR_ROLE_SNIPER
	};

	int GetDirectorRecommendedBombsite(CCSBot *bot);
	DirectorRoleType GetDirectorRole(CCSBot *bot);
	const char *GetDirectorRoleName(DirectorRoleType role) const;
...
private:
	...
	enum DirectorPlanType
	{
		DIRECTOR_PLAN_NONE = 0,
		DIRECTOR_PLAN_ATTACK_A,
		DIRECTOR_PLAN_ATTACK_B,
		DIRECTOR_PLAN_SPLIT_ATTACK,
		DIRECTOR_PLAN_DEFAULT_HOLD,
		DIRECTOR_PLAN_STACK_A,
		DIRECTOR_PLAN_STACK_B,
		DIRECTOR_PLAN_BALANCED_DEFENSE
	};

	void ResetDirector();
	void BuildDirectorRoundPlan();
	void UpdateDirectorDebug();
	void AnnounceDirectorText(const char *fmt, ...);
	void RecordDirectorRoundEvent(GameEventType event, CBaseEntity *pEntity, CBaseEntity *pOther);
	void UpdateDirectorAdaptiveDifficulty();
	DirectorRoleType ChooseDirectorRole(CCSBot *bot, int teamRoleIndex) const;
	const char *GetDirectorPlanName(DirectorPlanType plan) const;
	...
	int m_defenseRushSeed;					// stable per-round seed for local defender rush groups

	DirectorPlanType m_directorTPlan;
	DirectorPlanType m_directorCTPlan;
	int m_directorTBombsite;
	int m_directorCTStrongBombsite;
	int m_directorRole[MAX_CLIENTS + 1];
	int m_directorTargetZone[MAX_CLIENTS + 1];
	bool m_directorFollowsAdvice[MAX_CLIENTS + 1];
	bool m_directorDebugAnnounced;
	float m_directorNextDebugTime;
	bool m_directorRoundHadPlant;
	int m_directorRoundPlantedZone;
	int m_directorLastWinner;
	bool m_directorLastHadPlant;
	int m_directorLastPlantedZone;
	int m_directorLastAttackZone;
	int m_directorLastCTStrongZone;
	int m_directorTFailures[MAX_ZONES];
	int m_directorTSuccesses[MAX_ZONES];
	char m_directorReason[160];
	char m_directorLastSummary[160];
	int m_directorDifficultyOffset;
	int m_directorEffectiveDifficulty15;
	int m_directorRoundHumanKills;
	int m_directorRoundHumanDeaths;
	int m_directorLastHumanKills;
	int m_directorLastHumanDeaths;
	char m_directorDifficultySummary[160];
