...
void BotFollowMeme::Interpret(CCSBot *pSender, CCSBot *pReceiver) const
{
	...
	float followChance = 35.0f + 45.0f * pReceiver->GetProfile()->GetTeamwork();
	followChance += 12.0f * pReceiver->GetProfile()->GetAggression();
	followChance -= 18.0f * (1.0f - pReceiver->GetMorale());
	if (travelDistance > 550.0f)
		followChance -= 18.0f;

	if (RANDOM_FLOAT(0.0f, 100.0f) > followChance)
		return;
