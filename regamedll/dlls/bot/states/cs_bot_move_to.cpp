...
void MoveToState::OnUpdate(CCSBot *me)
{
	...
	{
		const float nearGoalRange = 80.0f;
		Vector toGoal = m_goalPosition - me->pev->origin;
		toGoal.z = m_goalPosition.z - me->GetFeetZ();
		if (!toGoal.IsLengthLessThan(nearGoalRange) && gpGlobals->time >= m_nextRepathTime && m_repathAttempts < 3)
		{
			RouteType route = m_routeType;
			if (me->GetTask() == CCSBot::FIND_TICKING_BOMB ||
				me->GetTask() == CCSBot::DEFUSE_BOMB ||
				me->GetTask() == CCSBot::MOVE_TO_LAST_KNOWN_ENEMY_POSITION)
			{
				route = FASTEST_ROUTE;
			}

			if (me->ComputePath(TheNavAreaGrid.GetNavArea(&m_goalPosition), &m_goalPosition, route))
			{
				m_repathAttempts++;
				m_nextRepathTime = gpGlobals->time + 0.35f;
				me->PrintIfWatched("Repathing to current objective instead of changing plans.\n");
				return;
			}
		}
