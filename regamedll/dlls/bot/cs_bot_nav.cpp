...
static bool IsGradualStepUpAhead(CCSBot *bot, const Vector &aheadRay, float lookAheadRange, float traceFraction, float targetGround)
{
	if (!bot || traceFraction <= 0.0f)
		return false;

	const float feetZ = bot->GetFeetZ();
	const float targetDz = targetGround - feetZ;
	if (targetDz <= StepHeight)
		return false;

	float checkRange = lookAheadRange * traceFraction - 8.0f;
	checkRange = Q_max(10.0f, Q_min(checkRange, lookAheadRange * 0.75f));

	Vector checkPos = bot->pev->origin + checkRange * aheadRay;
	checkPos.z += HalfHumanHeight;

	float intermediateGround;
	if (!bot->GetSimpleGroundHeightWithFloor(&checkPos, &intermediateGround))
		return false;

	const float intermediateDz = intermediateGround - feetZ;

	// Stairs/ramped steps show usable ground before the higher point; ledges stay low until blocked.
	return intermediateDz > StepHeight * 0.45f && intermediateDz < targetDz - 4.0f;
}
...
void CCSBot::MoveTowardsPosition(const Vector *pos)
{
		...
		if (IsRunning())
		{
			...
			if (GetSimpleGroundHeightWithFloor(&stepAhead, &ground, &normal))
			{
				if (normal.z > 0.9f)
				{
					...
					if (dz > StepHeight && dz < JumpCrouchHeight)
					{
						// Keep run-up momentum only for real blocking ledges, not ordinary steps.
						TraceResult ledgeTrace;
						Vector ledgeFrom = pev->origin + Vector(0.0f, 0.0f, StepHeight + 2.0f);
						Vector ledgeTo = ledgeFrom + farLookAheadRange * aheadRay;
						UTIL_TraceHull(ledgeFrom, ledgeTo, ignore_monsters, human_hull, ENT(pev), &ledgeTrace);

						if (!ledgeTrace.fStartSolid && ledgeTrace.flFraction < 0.85f
							&& !IsGradualStepUpAhead(this, aheadRay, farLookAheadRange, ledgeTrace.flFraction, ground))
		...
		if (!jumped)
		{
			...
			if (GetSimpleGroundHeightWithFloor(&stepAhead, &ground))
			{
				...
				if (dz > StepHeight)
				{
					TraceResult ledgeTrace;
					Vector ledgeFrom = pev->origin + Vector(0.0f, 0.0f, StepHeight + 2.0f);
					Vector ledgeTo = ledgeFrom + lookAheadRange * aheadRay;
					UTIL_TraceHull(ledgeFrom, ledgeTo, ignore_monsters, human_hull, ENT(pev), &ledgeTrace);

					if (!ledgeTrace.fStartSolid && ledgeTrace.flFraction < 0.9f
						&& !IsGradualStepUpAhead(this, aheadRay, lookAheadRange, ledgeTrace.flFraction, ground))
						jumped = DiscontinuityJump(ground);
				}
				else
				{
	...
	{
		const bool preserveJumpMomentum = m_isJumpCrouching && pev->velocity.Make2D().Length() > 80.0f;
		if (!preserveJumpMomentum)
			MoveBackward();
	}
