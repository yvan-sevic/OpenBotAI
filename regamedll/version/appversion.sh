...
init()
{
	...
	# Get commit id
	COMMIT_SHA=$(git -C "$GIT_DIR/" rev-parse --verify HEAD 2>/dev/null)
	if [ $? -ne 0 -o "$COMMIT_SHA" = "" ]; then
		COMMIT_SHA=unknown
	else
	...
	# Get remote url
	COMMIT_URL=$(git -C "$GIT_DIR/" config remote.$BRANCH_REMOTE.url 2>/dev/null)
	if [ $? -ne 0 -o "$COMMIT_URL" = "" ]; then
		COMMIT_URL=
	fi
	...
	# Always update appversion.h so APP_COMMIT_DATE/TIME reflect current build time,
	# even when the semantic version string hasn't changed.
	update_appversion
}
